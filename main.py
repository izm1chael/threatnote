...
def enrich_pipeline(json_str):
    try:
        ...
        # If either indicator or org_id is missing, raise a ValueError
        if not indicator or not org_id:
            raise ValueError("Missing required fields in the provided JSON.")
        
        # Query for Organization data
        org = Organization.query.filter_by(id=org_id).first()
        
        # If Organization doesn't exist, raise an error
        if not org:
            raise ValueError(f"No organization found with id {org_id}")
        
        # Convert the org object to a dictionary and remove unwanted attributes
        data = org.__dict__
        ...
        # Update the indicators in the database
        ...
        # For now, just re-raising the exception with a custom message
        app.logger.error(f"An error occurred while processing the pipeline: {str(e)}")
...
def custom_search():
    ...
    # Validate input
    if not query_string:
        return jsonify(success=False, message="Query string cannot be empty."), 400

    # Limiting the length of the query string for performance and security reasons
    if len(query_string) > 100:  
        return jsonify(success=False, message="Query string is too long."), 400

    try:
        reports_result = (
            Reports.query.filter(Reports.content.like('%' + query_string + '%'))
            .order_by(Reports.id)
            .limit(100)  # Limiting the number of results for performance reasons
            .all()
        )

        req_result = (
            Requirements.query.filter(Requirements.summary.like('%' + query_string + '%'))
            .order_by(Requirements.id)
            .limit(100)  # Limiting the number of results for performance reasons
            .all()
        )
        ...
        # Logging the error can be beneficial for debugging
        app.logger.error(f"Error during custom search: {e}")
...
def add_comment(obj_type=None, obj_id=None, user=None, comment=None):
    # Try to fetch data from form if not provided as arguments
    obj_type = obj_type or request.form.get('obj_type')
    obj_id = obj_id or request.form.get('obj_id')
    user = user or request.form.get('user', current_user.id)
    comment = comment or request.form.get('comment')
    
    valid_obj_types = {'report', 'requirement', 'req', 'indicator'}
    
    # Check if all required parameters are present and valid
    if not all([obj_type, obj_id, user, comment]):
        return jsonify(success=False, message="Missing parameters."), 400

    if obj_type not in valid_obj_types:
        return jsonify(success=False, message=f"Invalid obj_type. Valid types are: {', '.join(valid_obj_types)}"), 400

    new_comment = Comments(user=user, comment=comment)
    
    # Assign the comment to the right object based on obj_type
    if obj_type == 'report':
        new_comment.report = obj_id
    elif obj_type in ['requirement', 'req']:
        new_comment.requirement = obj_id
    else:  # obj_type == 'indicator'
    ...
    try:
        ...
        # Logging the error can be beneficial for debugging
        app.logger.error(f"Error while adding comment: {e}")
        return jsonify(success=False, message="Comment insert failed."), 500
